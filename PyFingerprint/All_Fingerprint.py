...
import numpy as np
from PyFingerprint.CDK_Fingerprint import cdk_fingerprint, cdk_fingerprints
from PyFingerprint.RDK_Fingerprint import rdk_fingerprint, rdk_fingerprints

try:
    from PyFingerprint.Babel_Fingerprint import ob_fingerprint, ob_fingerprints
except:
...
def get_fingerprint(smi, fp_type, nbit=None, depth=None, output='bit'):
    if fp_type in ["daylight", "extended", "graph", "pubchem", "estate",
                   "hybridization", "lingo", "klekota-roth", "shortestpath",
                   "signature", "circular", "Morgan", "cdk"]:
        if nbit is None:
            nbit = 1024
        if depth is None:
            depth = 6
...
def get_fingerprints(smis : list, fp_type, nbit=None, depth=None, output='bit'):
    """ Get plural fingerprints"""
    if fp_type in ["daylight", "extended", "graph", "pubchem", "estate",
                   "hybridization", "lingo", "klekota-roth", "shortestpath",
                   "signature", "circular", "Morgan", "cdk"]:
        if nbit is None:
        ...
        if depth is None:
            depth = 6
        res = cdk_fingerprints(smis, fp_type, nbit, depth, output)
    elif fp_type in ["rdkit", "maccs", "AtomPair", "TopologicalTorsion", "Avalon"]:
        res = rdk_fingerprints(smis, fp_type, nbit, output)
    elif fp_type in ["FP2", "FP3", "FP4"]:
        if nbit is None:
            nbit = 307
        res = ob_fingerprints(smis, fp_type, nbit, output)
    else:
        raise IOError('invalid fingerprint type')
    return res
